      ...
      class PreferenceMastersController < ApplicationController
        ...
        def do_GET(req, res)
          ...
          records = DB.client.select(
            'SELECT id, user_id, name FROM preference_masters WHERE user_id = ? ORDER BY id ASC',
            [user_id],
          )
        ...
        def do_POST(req, res)
          ...
          data = payload && payload[:preference_master]
          ...
          new_id = DB.client.execute(
            'INSERT INTO preference_masters (user_id, name, created_at, updated_at) VALUES (?, ?, NOW(), NOW())',
            [user_id, name],
          )

          record = DB.client.select(
            'SELECT id, user_id, name FROM preference_masters WHERE id = ?',
            [new_id],
          ).first
          ...
          case e.error_number
          when 1062
            render_json(res, status: 409, body: { error: '同じ名前の嗜好が既に登録されています。' })
          when 1452
